/**
 * Credit: https://github.com/e-hein/emulate-tab
 */
...
class NoSelectableElementFoundError extends Error {
  constructor() {
    super("no selectable elements found");
  }
}
...
export namespace emulateTab {
  ...
  export const to = (
    target: HTMLElement,
    sendKeyEventsWithShiftModifier = false,
  ) =>
    emulateTabFrom(activeElement()).to(target, sendKeyEventsWithShiftModifier);
  ...
  export const backwards = () =>
    to(getPreviousElement(activeElement()), shiftModifier);
   ...
   */
  export const findSelectableElements: () => HTMLElement[] =
    findAllElementsSelectableByTab;
}
...
function emulateTabFrom(source: HTMLElement = document.body) {
  return {
     ...
     */
    toPreviousElement: () =>
      emulateTabFromSourceToTarget(source, getPreviousElement(), shiftModifier),
     ...
     */
    toNextElement: () =>
      emulateTabFromSourceToTarget(source, getNextElement(source)),
     ...
     * @returns true if switched elemenet and false if default action had been prevented
     */
    to: (target: HTMLElement, sendKeyEventsWithShiftModifier = false) =>
      emulateTabFromSourceToTarget(
        source,
        target,
        sendKeyEventsWithShiftModifier,
      ),
  };
}
...
function emulateTabFromSourceToTarget(
  ...
  const executeDefaultAction = source.dispatchEvent(
    createTabEvent("keydown", sendKeyEventsWithShiftModifier),
  );
  if (executeDefaultAction) {
    ...
    if (
      ...
      selectableTypes.test(target.type)
    ) {
    ...
    source.dispatchEvent(
      createTabEvent("keypress", sendKeyEventsWithShiftModifier),
    );
    source.dispatchEvent(
      createTabEvent("keyup", sendKeyEventsWithShiftModifier),
    );
    return false;
  }
}
...
function getNextElement(lastElement: HTMLElement = document.body): HTMLElement {
  const selectableElements = findAllElementsSelectableByTab();
  if (selectableElements.length < 1) {
    throw new NoSelectableElementFoundError();
  }
  ...
  const currentIndex = selectableElements.indexOf(lastElement);
  const nextIndex =
    currentIndex + 1 < selectableElements.length ? currentIndex + 1 : 0;
...
function findAllElementsSelectableByTab() {
  ...
  const tabGroups = allElements
    ...
    .reduce(
      ...
      {} as {
        [tabIndex: string]: { tabIndex: number; elements: HTMLElement[] };
      },
    );
...
const isVisibleBySize =
  (sizeAttribute: keyof HTMLElement) => (element: HTMLElement) => {
    const size: any = element[sizeAttribute];
    return !!size && typeof size === "number" && size > 0;
  };

function findSizeAttributeKey(
  element: Element = document.body,
): keyof HTMLElement | undefined {
  const htmlElement = element as HTMLElement;
  const sizeAttributeKey = possibleSizeAttributeKeys.find((key) => {
...
function hasValidTabIndex(
  element: HTMLElement,
): element is HTMLElementWithValidTabIndex {
  return typeof element.tabIndex === "number" && element.tabIndex >= 0;
}

const isNotDisabledInput = (element: HTMLElement) =>
